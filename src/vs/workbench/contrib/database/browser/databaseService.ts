...
import { Database, DBRecord, Field, STATUS_OPTIONS } from '../common/database.js';
...
export interface IDatabaseService {
	...
	createDatabase(name: string, folderUri: URI): Promise<{ db: Database; uri: URI }>;
...
class DatabaseService extends Disposable implements IDatabaseService {
	...
	async createDatabase(name: string, folderUri: URI): Promise<{ db: Database; uri: URI }> {
		const db = isTasksDatabaseName(name) ? createTaskDatabaseTemplate(name) : createDefaultDatabaseTemplate(name);
		const safeName = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
		const uri = URI.joinPath(folderUri, `${safeName}.db.json`);
...
function coerceValueForField(field: Field, value: unknown): string | number | boolean | string[] | null {
	...
	switch (field.type) {
		...
		case 'status': {
			...
			const candidate = normalized == null ? null : String(normalized);
			if (!candidate) {
				return null;
			}
